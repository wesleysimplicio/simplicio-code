...
pub enum HbiError {
    ...
    #[error("HBI schema id is not valid UTF-8")]
    InvalidSchema,
...
pub fn encode_hbi(schema: &str, sections: &[HbiSection]) -> Result<Vec<u8>, HbiError> {
    ...
    for section in sections {
        ...
        output[payload_offset..payload_offset + section.bytes.len()]
...
impl<'a> HbiReader<'a> {
    pub fn open(bytes: &'a [u8]) -> Result<Self, HbiError> {
        ...
        if total_len != bytes.len() {
            return Err(HbiError::TotalLength {
                declared: total_len,
                actual: bytes.len(),
            });
        }
        ...
        let fingerprint = blake3::hash(schema_bytes);
        if &bytes[28..60] != fingerprint.as_bytes() {
            return Err(HbiError::SchemaFingerprintMismatch);
        }
        ...
        for index in 0..section_count {
            ...
            let end_offset = offset
                .checked_add(length)
                .ok_or(HbiError::SectionOutOfBounds { index })?;
            if offset < header_len || end_offset > total_len {
                return Err(HbiError::SectionOutOfBounds { index });
            }
            let checksum: [u8; 32] = bytes[directory_offset + 24..directory_offset + 56]
                .try_into()
                .unwrap();
            ...
            if blake3::hash(&bytes[range.clone()]).as_bytes() != checksum.as_ref() {
                return Err(HbiError::SectionChecksumMismatch { index });
            }
            ranges.push(range);
...
mod tests {
    ...
    fn round_trip_and_checksum_validation() {
        ...
        *corrupt.last_mut().unwrap() ^= 1;
        assert!(matches!(
            ...
            Err(HbiError::SectionChecksumMismatch { .. })
        ));
    }
    ...
    fn rejects_truncation_and_overlapping_sections() {
        ...
        .unwrap();
        assert!(matches!(
            HbiReader::open(&bytes[..bytes.len() - 1]),
            Err(HbiError::TotalLength { .. })
        ));
        let mut overlap = bytes;
        let directory = HEADER_SIZE + "schema".len();
        let first_offset =
            u64::from_le_bytes(overlap[directory + 8..directory + 16].try_into().unwrap());
        overlap[directory + DIRECTORY_ENTRY_SIZE + 8..directory + DIRECTORY_ENTRY_SIZE + 16]
            .copy_from_slice(&first_offset.to_le_bytes());
        assert!(matches!(
            HbiReader::open(&overlap),
            Err(HbiError::OverlappingSections)
        ));
    }
}
