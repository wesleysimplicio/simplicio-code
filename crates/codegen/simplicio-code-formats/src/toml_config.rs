...
pub fn parse_code_config(input: &str) -> Result<CodeConfig, ConfigError> {
    ...
    let root = value
        .as_table()
        .ok_or_else(|| ConfigError::Parse("root must be a table".into()))?;
    for key in root.keys() {
        if !matches!(key.as_str(), "schema_version" | "runtime") {
    ...
    if let Some(runtime) = root.get("runtime") {
        let table = runtime
            .as_table()
            .ok_or_else(|| ConfigError::Parse("runtime must be a table".into()))?;
        for key in table.keys() {
            if !matches!(key.as_str(), "handshake_timeout_ms" | "max_file_bytes") {
                return Err(ConfigError::UnknownKey(format!("runtime.{key}")));
            }
        }
    }
    let config: CodeConfig = value
        .try_into()
        .map_err(|e| ConfigError::Parse(e.to_string()))?;
    if config.schema_version != 1 {
        return Err(ConfigError::Version(config.schema_version));
    }
    Ok(config)
}
...
mod tests {
    ...
    fn typed_toml_defaults_and_rejects_unknown_keys() {
        ...
        assert!(matches!(
            parse_code_config("schema_version = 1\nunknown = true\n"),
