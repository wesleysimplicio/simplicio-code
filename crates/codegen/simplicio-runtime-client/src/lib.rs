...
mod tests {
    ...
    #[test]
    fn prototype_artifact_namespace_rejects_path_injection() {
        assert!(safe_artifact_id("candidate-1"));
        assert!(!safe_artifact_id("../secret"));
        assert!(!safe_artifact_id("candidate/secret"));
        assert!(!safe_artifact_id("candidate\u{1b}[31m"));
        assert_eq!(
            PROTOTYPE_ARTIFACT_ROOT,
            ".simplicio/artifacts/prototype-first"
        );
    }
