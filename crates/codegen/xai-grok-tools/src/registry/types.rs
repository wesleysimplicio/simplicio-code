Warning: truncated output (original token count: 47420)
Total output lines: 4531
...
pub fn generate_schema<T: schemars::JsonSchema>() -> serde_json::Value {
        ...
        && obj.get("type").and_then(|v| v.as_str()) == Some("object")
    {
        ...
        obj.entry("required")
            .or_insert_with(|| serde_j…7420 tokens truncated…ed SearchReplace(NoMatchesFound), got: {other:?}"),
        }
    }
