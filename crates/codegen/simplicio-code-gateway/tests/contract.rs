...
use simplicio_code_gateway::{
    DeviceAuthorization, Entitlement, GatewayModel, GatewayUsage, PUBLIC_MODEL_ID,
};

#[test]
fn models_response_fixture_matches_gateway_model_schema() {
    let raw = include_str!("fixtures/models_response.json");
    let parsed: Vec<GatewayModel> =
        serde_json::from_str(raw).expect("fixture must deserialize into Vec<GatewayModel>");
    assert_eq!(parsed.len(), 1);
    assert_eq!(
        parsed[0].id, PUBLIC_MODEL_ID,
        "simplicio-1 must be the only publicly-named model"
    );
}
...
fn usage_response_fixture_matches_gateway_usage_schema() {
    ...
    let parsed: GatewayUsage =
        serde_json::from_str(raw).expect("fixture must deserialize into GatewayUsage");
...
fn entitlement_response_fixture_matches_entitlement_schema_and_hides_provider() {
    ...
    let parsed: Entitlement =
        serde_json::from_str(raw).expect("fixture must deserialize into Entitlement");
...
fn device_authorization_fixture_matches_schema() {
    ...
    let parsed: DeviceAuthorization =
        serde_json::from_str(raw).expect("fixture must deserialize into DeviceAuthorization");
...
fn openapi_schema_declares_every_path_this_crate_actually_calls() {
    ...
    for path in [
        ...
        assert!(
            raw.contains(path),
            "openapi schema missing path used by the client: {path}"
        );
    }
    assert!(raw.contains(PUBLIC_MODEL_ID));
    assert!(
        !raw.to_lowercase().contains("openrouter"),
        "schema must never name an upstream provider"
    );
    assert!(
        !raw.to_lowercase().contains("x.ai"),
        "schema must never name an upstream provider host"
    );
}
...
fn openapi_bootstrap_device_endpoints_override_global_bearer_security() {
    let raw = include_str!("../openapi/simplicio-gateway.yaml");
    for path in [
        "/v1/code/auth/device/authorize:",
        "/v1/code/auth/device/token:",
    ] {
        let section = raw
            .split_once(path)
            .expect("device endpoint must be declared")
            .1;
        ...
        assert!(
            ...
            "bootstrap endpoint must be anonymous: {path}"
        );
    }
}
