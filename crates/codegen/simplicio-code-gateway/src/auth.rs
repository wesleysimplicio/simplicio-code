...
impl OsSecretStore {
    pub fn new(username: impl Into<String>) -> Self {
        Self { service: "simplicio-code".into(), username: username.into() }
    }
...
impl SecretStore for OsSecretStore {
    ...
    fn save_refresh_token(&self, token: &Secret) -> Result<(), AuthError> {
        self.entry()?.set_password(token.expose()).map_err(|e| AuthError::Storage(e.to_string()))
    }
...
impl SecretStore for MemorySecretStore {
    fn load_refresh_token(&self) -> Result<Option<Secret>, AuthError> {
        self.0.lock().map_err(|_| AuthError::Storage("lock poisoned".into())).map(|v| v.clone())
    }
    fn save_refresh_token(&self, token: &Secret) -> Result<(), AuthError> {
        *self.0.lock().map_err(|_| AuthError::Storage("lock poisoned".into()))? = Some(token.clone());
        Ok(())
    }
    fn clear_refresh_token(&self) -> Result<(), AuthError> {
        *self.0.lock().map_err(|_| AuthError::Storage("lock poisoned".into()))? = None;
        Ok(())
    }
}
...
impl Entitlement {
    pub fn is_valid_at(&self, now: DateTime<Utc>) -> bool { now + CLOCK_SKEW < self.expires_at }
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub enum AuthState { SignedOut, Authorized, Revoked }
...
impl<S: SecretStore> AuthSession<S> {
    pub fn new(store: Arc<S>) -> Self {
        Self { store, inner: Mutex::new(SessionInner { access_token: None, access_expires_at: None, entitlement: None, state: AuthState::SignedOut }) }
    }

    pub fn state(&self) -> AuthState { self.inner.lock().map(|v| v.state.clone()).unwrap_or(AuthState::Revoked) }

    pub fn install(&self, token: TokenResponse, entitlement: Entitlement, now: DateTime<Utc>) -> Result<(), AuthError> {
        if !entitlement.is_valid_at(now) || token.expires_in == 0 { return Err(AuthError::EntitlementRequired); }
        if let Some(refresh) = token.refresh_token.as_ref() { self.store.save_refresh_token(&Secret::new(refresh.expose()))?; }
        let mut inner = self.inner.lock().map_err(|_| AuthError::Storage("lock poisoned".into()))?;
    ...
    pub fn access_token(&self, now: DateTime<Utc>) -> Result<SecretString, AuthError> {
        let inner = self.inner.lock().map_err(|_| AuthError::Storage("lock poisoned".into()))?;
        if inner.state == AuthState::Revoked { return Err(AuthError::Revoked); }
        if inner.state != AuthState::Authorized { return Err(AuthError::EntitlementRequired); }
        if !inner.entitlement.as_ref().is_some_and(|e| e.is_valid_at(now)) { return Err(AuthError::EntitlementRequired); }
        if inner.access_expires_at.is_none_or(|e| now + CLOCK_SKEW >= e) { return Err(AuthError::AccessExpired); }
    ...
    pub fn entitlement(&self, now: DateTime<Utc>) -> Result<Entitlement, AuthError> {
        let inner = self.inner.lock().map_err(|_| AuthError::Storage("lock poisoned".into()))?;
        if inner.state != AuthState::Authorized || !inner.entitlement.as_ref().is_some_and(|e| e.is_valid_at(now)) { return Err(AuthError::EntitlementRequired); }
        inner.entitlement.clone().ok_or(AuthError::EntitlementRequired)
    }

    pub fn rotate_refresh(&self, token: &TokenResponse, now: DateTime<Utc>) -> Result<(), AuthError> {
        let refresh = token.refresh_token.as_ref().ok_or_else(|| AuthError::Protocol("refresh response omitted rotated refresh token".into()))?;
        let entitlement = self.entitlement(now)?;
        self.store.save_refresh_token(&Secret::new(refresh.expose()))?;
        let mut inner = self.inner.lock().map_err(|_| AuthError::Storage("lock poisoned".into()))?;
...
impl<S: SecretStore> IdentityClient<S> {
    ...
    pub fn with_http_client(endpoints: AuthEndpoints, store: Arc<S>, http: reqwest::Client) -> Self {
        Self { http, endpoints, session: Arc::new(AuthSession::new(store)) }
    }

    pub async fn begin_device_authorization(&self) -> Result<DeviceAuthorization, AuthError> {
        let response = self.http.post(self.endpoints.url(paths::DEVICE_AUTHORIZE)).json(&serde_json::json!({"scope": "code"})).send().await?;
        decode_response(response).await
    }

    pub async fn poll_device_authorization(&self, device: &DeviceAuthorization, cancel: CancellationToken, now: DateTime<Utc>) -> Result<TokenResponse, AuthError> {
        let deadline = now + Duration::seconds(device.expires_in as i64);
        loop {
            if cancel.is_cancelled() { return Err(AuthError::Cancelled); }
            if Utc::now() + CLOCK_SKEW >= deadline { return Err(AuthError::DeviceExpired); }
            let response = self.http.post(self.endpoints.url(paths::DEVICE_TOKEN)).json(&serde_json::json!({"device_code": device.device_code.expose()})).send().await?;
            if response.status() == StatusCode::ACCEPTED {
                sleep(TokioDuration::from_secs(device.interval.max(1))).await;
                continue;
            }
            if response.status() == StatusCode::FORBIDDEN { return Err(AuthError::DeviceDenied); }
    ...
    pub async fn fetch_entitlement(&self) -> Result<Entitlement, AuthError> {
        ...
        let response = self.http.get(self.endpoints.url(paths::ENTITLEMENT)).bearer_auth(token.expose()).send().await?;
        decode_response(response).await
    }
    ...
    pub async fn logout(&self) -> Result<(), AuthError> {
        ...
        if let Some(token) = token {
            let response = self.http.post(self.endpoints.url(paths::REVOKE)).bearer_auth(token.expose()).send().await?;
            if !response.status().is_success() { return Err(server_error(response).await); }
        }
...
async fn server_error(response: reqwest::Response) -> AuthError {
    ...
    let message = response.text().await.unwrap_or_else(|_| "identity request failed".into());
    AuthError::Server { status, message: crate::redaction::redact_diagnostics(&serde_json::json!({"message": message})).message }
}

async fn decode_response<T: for<'de> Deserialize<'de>>(response: reqwest::Response) -> Result<T, AuthError> {
    let status = response.status();
    if !status.is_success() { return Err(server_error(response).await); }
    response.json().await.map_err(|e| AuthError::Protocol(e.to_string()))
}
...
impl FakeIdentity {
    pub fn new(entitlement: Entitlement) -> Self {
        Self { state: Arc::new(Mutex::new(FakeIdentityState { pending_polls: 1, refresh_generation: 0, revoked: false, entitlement })) }
    }
    pub fn authorize(&self) -> DeviceAuthorization {
        DeviceAuthorization { device_code: SecretString::new("fake-device-code"), user_code: "FAKE-CODE".into(), verification_uri: Url::parse("https://login.simplicio.invalid/device").expect("fake URL"), expires_in: 600, interval: 0 }
    }
    pub fn poll(&self, device: &DeviceAuthorization) -> Result<TokenResponse, AuthError> {
        if device.device_code.expose() != "fake-device-code" { return Err(AuthError::Device("unknown device".into())); }
        let mut state = self.state.lock().map_err(|_| AuthError::Storage("lock poisoned".into()))?;
        if state.revoked { return Err(AuthError::Revoked); }
        if state.pending_polls > 0 { state.pending_polls -= 1; return Err(AuthError::Device("authorization_pending".into())); }
        state.refresh_generation += 1;
        Ok(TokenResponse { access_token: SecretString::new(format!("fake-access-{}", state.refresh_generation)), refresh_token: Some(SecretString::new(format!("fake-refresh-{}", state.refresh_generation))), expires_in: 3600, token_type: "Bearer".into() })
    }
    pub fn entitlement(&self) -> Result<Entitlement, AuthError> {
        let state = self.state.lock().map_err(|_| AuthError::Storage("lock poisoned".into()))?;
        if state.revoked { return Err(AuthError::Revoked); }
        Ok(state.entitlement.clone())
    }
    pub fn revoke(&self) -> Result<(), AuthError> {
        self.state.lock().map_err(|_| AuthError::Storage("lock poisoned".into())).map(|mut s| { s.revoked = true; })
    }
}
...
mod tests {
    ...
    fn entitlement(expires_at: DateTime<Utc>) -> Entitlement { Entitlement { plan: "pro".into(), expires_at, max_request_tokens: 1000, max_tool_calls: 4 } }
    fn token(access: &str, refresh: &str) -> TokenResponse { TokenResponse { access_token: SecretString::new(access), refresh_token: Some(SecretString::new(refresh)), expires_in: 300, token_type: "Bearer".into() } }
    ...
    fn access_is_memory_only_and_entitlement_fail_closed() {
        ...
        let now = Utc::now();
        assert!(matches!(session.access_token(now), Err(AuthError::EntitlementRequired)));
        session.install(token("access", "refresh"), entitlement(now + Duration::hours(1)), now).unwrap();
        assert_eq!(session.access_token(now).unwrap().expose(), "access");
        assert_eq!(store.load_refresh_token().unwrap().unwrap().expose(), "refresh");
        assert_eq!(format!("{:?}", session.access_token(now).unwrap()), "[REDACTED]");
    }
    ...
    fn expiry_skew_is_fail_closed() {
        ...
        session.install(near_expiry_access, entitlement(now + Duration::hours(1)), now).unwrap();
        assert!(matches!(session.access_token(now), Err(AuthError::AccessExpired)));
    }
    ...
    fn refresh_rotation_replaces_old_secret_and_logout_clears() {
        ...
        let session = AuthSession::new(store.clone());
        let now = Utc::now();
        session.install(token("one", "old"), entitlement(now + Duration::hours(1)), now).unwrap();
    ...
    fn os_secret_store_round_trips_a_real_secret_through_the_platform_keychain() {
        ...
        store.save_refresh_token(&Secret::new("integration-test-refresh-token")).unwrap();
        let loaded = store.load_refresh_token().unwrap().expect("secret must be readable back from the OS store");
        ...
        assert!(store.load_refresh_token().unwrap().is_none(), "clear_refresh_token must delete the OS credential entry");
    }
}
