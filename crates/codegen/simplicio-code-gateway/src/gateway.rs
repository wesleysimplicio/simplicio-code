...
impl GatewayLimits {
    pub fn enforce(&self, request: &ChatRequest) -> Result<(), GatewayError> {
        if request.estimated_tokens > self.max_request_tokens {
            return Err(GatewayError::LimitExceeded(format!(
                "request tokens {} > {}",
                request.estimated_tokens, self.max_request_tokens
            )));
        }
        if request.tools.len() > self.max_tool_calls as usize {
            return Err(GatewayError::LimitExceeded(format!(
                "tool calls {} > {}",
                request.tools.len(),
                self.max_tool_calls
...
impl<S: SecretStore + 'static> PrivateGateway<S> {
    ...
    pub async fn chat_stream(
        ...
        if request.model != PUBLIC_MODEL_ID {
            return Err(GatewayError::Rejected(format!(
                "only {PUBLIC_MODEL_ID} is public"
            )));
        }
        ...
        if !response.status().is_success() {
            ...
            return Err(GatewayError::Server {
                status,
                message: "private gateway request failed".into(),
                retry_after,
            });
        }
...
pub fn parse_sse_events(input: &[u8]) -> Result<(Vec<GatewayEvent>, Vec<u8>), GatewayError> {
    ...
    let mut offset = 0;
    while let Some((end, consumed)) = find_sse_frame_end(&input[offset..]) {
        let end = offset + end;
        ...
        if data == "[DONE]" {
            ...
            events.push(
                serde_json::from_str(&data).map_err(|e| GatewayError::Protocol(e.to_string()))?,
            );
        }
        offset += consumed;
    }
...
/// Finds the next blank SSE line while retaining an incomplete trailing CR.
/// This makes the function safe to call after every arbitrary network chunk:
/// both LF and CRLF framing work, including a CRLF split across two chunks.
fn find_sse_frame_end(input: &[u8]) -> Option<(usize, usize)> {
    let mut line_start = 0;
    let mut previous_terminator_start = None;
    let mut index = 0;
    while index < input.len() {
        let terminator_len = match input[index] {
            b'\n' => 1,
            b'\r' => {
        ...
        if index == line_start {
            let frame_end = previous_terminator_start.unwrap_or(0);
            return Some((frame_end, index + terminator_len));
        }
        previous_terminator_start = Some(index);
...
impl<S: SecretStore> FakeGateway<S> {
    pub fn new(session: Arc<AuthSession<S>>) -> Self {
        Self {
            session,
    ...
    pub fn models(&self) -> Result<Vec<GatewayModel>, GatewayError> {
        ...
        Ok(vec![GatewayModel {
            id: PUBLIC_MODEL_ID.into(),
            display_name: "Simplicio-1".into(),
    ...
    pub fn stream(
        ...
        if request.model != PUBLIC_MODEL_ID {
            return Err(GatewayError::Rejected("model is not public".into()));
        }
        limits.enforce(request)?;
        ...
        Ok(vec![
            GatewayEvent {
                id: Some("fake-request".into()),
                text_delta: Some("fake response".into()),
                tool_call: None,
                usage: Some(GatewayUsage {
                    request_tokens: request.estimated_tokens,
                    response_tokens: 2,
                    remaining_tokens: limits
                        .max_request_tokens
                        .saturating_sub(request.estimated_tokens),
                    remaining_tool_calls: limits
                        .max_tool_calls
                        .saturating_sub(request.tools.len() as u32),
                }),
                done: false,
            },
            GatewayEvent {
...
mod tests {
    ...
    fn parser_handles_partial_sse_frames_and_done() {
        ...
        let (events, _rest) =
            parse_sse_events(&[rest, b"\ndata: [DONE]\n\n".to_vec()].concat()).unwrap();
        assert_eq!(events.len(), 2);
        assert!(events[1].done);
    }

    #[test]
    fn parser_handles_fragmented_crlf_frames() {
        let first = b"data: {\"id\":\"r\",\"text_delta\":\"hi\",\"tool_call\":null,\"usage\":null,\"done\":false}\r";
        let (events, rest) = parse_sse_events(first).unwrap();
        assert!(events.is_empty());
        let second = [rest, b"\n\r\ndata: [DONE]\r\n\r\n".to_vec()].concat();
        let (events, rest) = parse_sse_events(&second).unwrap();
        assert_eq!(events.len(), 2);
